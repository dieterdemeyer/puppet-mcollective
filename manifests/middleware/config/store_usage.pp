define mcollective::middleware::config::store_usage($limit='1 gb') {
  ...
  augeas { 'systemUsage/storeUsage' :
    ...
    changes => [
      "set beans/broker/systemUsage/systemUsage/storeUsage/storeUsage/#attribute/name ${title}",
      "set beans/broker/systemUsage/systemUsage/storeUsage/storeUsage/#attribute/limit ${limit}",
    ],
    onlyif  => "match beans/broker/systemUsage/systemUsage/storeUsage/storeUsage[#attribute/limit[. =\"${limit}\"] and #attribute/name[. =\"${title}\"]] size == 0",
